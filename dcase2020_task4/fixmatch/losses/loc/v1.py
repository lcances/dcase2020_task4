...
class FixMatchLossMultiHotLocV1(FixMatchLossLocABC):
	""" FixMatch loss multi-hot (based on FixMatchLossMultiHotV1 class tagging). """
	...
	@staticmethod
	def from_args(args: Namespace) -> 'FixMatchLossMultiHotLocV1':
		return FixMatchLossMultiHotLocV1(args.lambda_u, args.threshold_confidence, args.threshold_multihot)
	...
	def __call__(
		...
		u_mask_confidence_weak = self.confidence_mask(u_pred_weak_augm_weak, dim=1)
		...
		u_mask_confidence_strong = self.confidence_mask(u_pred_strong_augm_weak, dim=(1, 2))
	...
	def confidence_mask(self, pred: Tensor, dim: Union[int, tuple]) -> Tensor:
		if type(dim) == int:
			...
			# Note : does not work for all tuples
			maxes = pred.clone()

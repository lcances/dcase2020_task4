...
directory_to_load = [
    os.path.join("DESED", "dataset", "audio", "train"),
...
for directory in directory_to_load:
    ...
    file_list = [name for name in file_list if name not in to_remove]
    dataset_shape = (len(file_list), SR*LENGTH)

    # for every file of the directory, extract the raw audio
    raw_audios = []
    workers = Pool(args.num_workers)
    folder_path = [folder_path] * len(file_list)

    results = workers.starmap(load_file, zip(folder_path, file_list))
